...
# -------------------------
def check_login():
    """Check if the user is logged in via session state"""
...
def main_app():
    ...
    if uploaded_file is not None:
        try:
            ...
            ws = Sample_sheet
            for col in range(len(unit_legal)):
                ws[f"A{col + 3}"] = AR_Aging_sheet["A7"].value
            ...
            def process_until_none_col(wb_obj, sheet_name, column):
                result = []
                ws_local = wb_obj[sheet_name]
                for row in range(8, ws_local.max_row + 1):
                    cell_value = ws_local[f"{column}{row}"].value
            ...
            resident_ar = process_until_none_col(wb, "AR Aging (excluding HUD)", 'B')
            status_ar = process_until_none_col(wb, "AR Aging (excluding HUD)", 'C')
            tenant_name_ar = process_until_none_col(wb, "AR Aging (excluding HUD)", 'D')
            _0_30_ar = process_until_none_col(wb, "AR Aging (excluding HUD)", 'F')
            total_charges_ar = process_until_none_col(wb, "AR Aging (excluding HUD)", 'E')
            ...
            for u, resident, status, tenant_name, total_charges, ar_0_30 in zip(
                ...
                key = str(u).strip().upper() if u else ""
                ar_data[key] = {
                    "resident": resident,
                    "status": status,
            ...
            def fetch_rent_amounts(wb_obj, sheet_name):
                ...
                for row in range(2, ws_local.max_row + 1):
                    ...
                    g_text = str(g_value).strip().lower() if g_value else ""
                    if "market" in g_text:
                        break
                    if "rent" in g_text:
                        if h_value and isinstance(h_value, (int, float)) and h_value >= 0:
            ...
            def fetch_name_rent(wb_obj, sheet_name, property_value=Sample_sheet['A3']):
                ws_local = wb_obj[sheet_name]
                name_rent_local = []
                for row in range(7, ws_local.max_row + 1):
            ...
            for unit in unit_legal:
                ...
                if key in unit_to_name:
                    ...
                    if key not in amount_written_units:
                        ws[f"I{current_row}"] = unit_to_amount.get(key, "")
            ...
            st.download_button(
                ...
                file_name="Modified_Custom_Report.xlsx",
